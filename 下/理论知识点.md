...
- 答：No！cus user pages may be invalid or cause page faults
- kernel code 为什么不能直接 `memcpy` user buffer？
- 答：No！cuz user pointers are untrusted and may be invalid，应该使用copy_from_user / copy_to_user
- process context 和 interrupt context 的区别。
- 答：前者可以sleep，后者不行，前者能使用mutex
